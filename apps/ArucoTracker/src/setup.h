...
void ofApp::setupGUI() {
  ...
  mBDebugMarkers->button->onButtonEvent(
      [&](ofxDatGuiButtonEvent v) { mArucoDetector->toggleMarkerInfo(); });
...
void ofApp::setupCalibration() {
  mArucoDetector->setupCalibration(DEFAULT_GRID_WIDTH, DEFAULT_GRID_HEIGHT);
...
void ofApp::setupGridDetector() {
  ...
  for (int i = 0; i < mNumInputs; i++) {
    ...
    ofFbo fbo;
    fbo.allocate(CAM_WIDTH, CAM_HEIGHT, GL_RGBA);
    fbo.begin();
    ofClear(0, 0, 0, 255);
    fbo.end();
