...
void Intrinsics::setup(float focalLength, cv::Size imageSize,
                       cv::Size2f sensorSize, cv::Point2d principalPoint) {
...
void Intrinsics::setup(Mat cameraMatrix, cv::Size imageSize,
...
void Intrinsics::updateValues() {
  calibrationMatrixValues(cameraMatrix, imageSize, sensorSize.width,
                          sensorSize.height, fov.x, fov.y, focalLength,
...
Mat Intrinsics::getCameraMatrix() const { return cameraMatrix; }
...
Point2d Intrinsics::getPrincipalPoint() const { return principalPoint; }

void Intrinsics::loadProjectionMatrix(float nearDist, float farDist,
                                      cv::Point2d viewportOffset) const {
  ofViewport(viewportOffset.x, viewportOffset.y, imageSize.width,
             imageSize.height);
  ...
  ofMatrix4x4 lookAt;
  lookAt.makeLookAtViewMatrix(ofVec3f(0, 0, 0), ofVec3f(0, 0, 1),
                              ofVec3f(0, -1, 0));
  ofMultMatrix(lookAt);
}
...
Calibration::Calibration()
    : patternType(CHESSBOARD),
      patternSize(cv::Size(10, 7)), // based on Chessboard_A4.pdf, assuming
                                    // world units are centimeters
      subpixelSize(cv::Size(11, 11)), squareSize(2.5), reprojectionError(0),
      distCoeffs(Mat::zeros(8, 1, CV_64F)), fillFrame(true), ready(false) {}

void Calibration::save(string filename, bool absolute) const {
  if (!ready) {
    ofLog(OF_LOG_ERROR, "Calibration::save() failed, because your calibration "
                        "isn't ready yet!");
  }
...
void Calibration::setDistortionCoefficients(float k1, float k2, float p1,
...
void Calibration::setFillFrame(bool fillFrame) { this->fillFrame = fillFrame; }
...
bool Calibration::add(Mat img) {
  ...
  if (found)
    ...
    ofLog(OF_LOG_ERROR, "Calibration::add() failed, maybe your patternSize is "
                        "wrong or the image has poor lighting?");
  return found;
}
bool Calibration::findBoard(Mat img, vector<Point2f> &pointBuf, bool refine) {
  bool found = false;
  if (patternType == CHESSBOARD) {
    // no CV_CALIB_CB_FAST_CHECK, because it breaks on dark images (e.g., dark
    // IR images from kinect)
    int chessFlags =
        cv::CALIB_CB_ADAPTIVE_THRESH; // | CV_CALIB_CB_NORMALIZE_IMAGE;
    ...
    if (found) {
      ...
      if (refine) {
        ...
        // in other words, if your smallest square is 11x11 pixels, then set
        // this to 11x11
        cornerSubPix(
            grayMat, pointBuf, subpixelSize, cv::Size(-1, -1),
            TermCriteria(cv::TermCriteria::EPS + cv::TermCriteria::MAX_ITER, 30,
                         0.1));
      }
    }
  }
    ...
    int flags = (patternType == CIRCLES_GRID
...
bool Calibration::clean(float minReprojectionError) {
  ...
  if (size() > 0) {
    ...
    ofLog(OF_LOG_ERROR,
          "Calibration::clean() removed the last object/image point pair");
    return false;
  }
}
bool Calibration::calibrate() {
  if (size() < 1) {
    ofLog(OF_LOG_ERROR, "Calibration::calibrate() doesn't have any image data "
                        "to calibrate from.");
    if (ready) {
      ofLog(OF_LOG_ERROR, "Calibration::calibrate() doesn't need to be called "
                          "after Calibration::load().");
    }
    return ready;
  }
  ...
  int calibFlags = 0;
  float rms = calibrateCamera(objectPoints, imagePoints, addedImageSize,
                              cameraMatrix, distCoeffs, boardRotations,
                              boardTranslations, calibFlags);
  ofLog(OF_LOG_VERBOSE,
        "calibrateCamera() reports RMS error of " + ofToString(rms));

  ready = checkRange(cameraMatrix) && checkRange(distCoeffs);

  if (!ready) {
    ofLog(OF_LOG_ERROR,
          "Calibration::calibrate() failed to calibrate the camera");
  }
...
bool Calibration::isReady() { return ready; }
...
ofVec2f Calibration::undistort(ofVec2f &src) const {
  ...
  ;
  undistortPoints(matSrc, matDst, distortedIntrinsics.getCameraMatrix(),
                  distCoeffs);
  return dst;
}
...
bool Calibration::getTransformation(Calibration &dst, Mat &rotation,
                                    Mat &translation) {
  // if(imagePoints.size() == 0 || dst.imagePoints.size() == 0) {
  if (!ready) {
    ofLog(OF_LOG_ERROR, "getTransformation() requires both Calibration objects "
                        "to have just been calibrated");
    return false;
  }
  if (imagePoints.size() != dst.imagePoints.size() ||
      patternSize != dst.patternSize) {
    ofLog(OF_LOG_ERROR, "getTransformation() requires both Calibration objects "
                        "to be trained simultaneously on the same board");
    return false;
  }
  ...
  stereoCalibrate(objectPoints, imagePoints, dst.imagePoints, cameraMatrix,
                  distCoeffs, dstCameraMatrix, dst.distCoeffs,
...
Mat Calibration::getDistCoeffs() const { return distCoeffs; }
int Calibration::size() const { return imagePoints.size(); }
...
/*
 ...
 src.push_back(ofVec2f(
                                    ofMap(x, -1, divisions.x, 0,
 addedImageSize.width),
                                    ofMap(y, -1, divisions.y, 0,
 addedImageSize.height)));
 }
 }
 undistort(src, dst);
 ofMesh mesh;
...
void Calibration::updateObjectPoints() {
  vector<Point3f> points =
      createObjectPoints(patternSize, squareSize, patternType);
...
void Calibration::updateReprojectionError() {
  ...
  for (int i = 0; i < (int)objectPoints.size(); i++) {
    projectPoints(Mat(objectPoints[i]), boardRotations[i], boardTranslations[i],
                  ...
                  imagePoints2);
    double err = norm(Mat(imagePoints[i]), Mat(imagePoints2), cv::NORM_L2);
    int n = objectPoints[i].size();
    perViewErrors[i] = sqrt(err * err / n);
    totalErr += err * err;
    totalPoints += n;
    ofLog(OF_LOG_VERBOSE, "view " + ofToString(i) + " has error of " +
                              ofToString(perViewErrors[i]));
  }
...
void Calibration::updateUndistortion() {
  ...
  initUndistortRectifyMap(distortedIntrinsics.getCameraMatrix(), distCoeffs,
                          ...
                          distortedIntrinsics.getImageSize(), CV_16SC2,
...
vector<Point3f>
Calibration::createObjectPoints(cv::Size patternSize, float squareSize,
