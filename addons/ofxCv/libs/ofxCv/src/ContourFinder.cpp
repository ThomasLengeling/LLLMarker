...
struct CompareContourArea {
  CompareContourArea(const std::vector<double> &areaVec) : mAreaVec(areaVec) {}
...
ContourFinder::ContourFinder()
    : autoThreshold(true), invert(false), simplify(true), thresholdValue(128.),
      useTargetColor(false), contourFindingMode(cv::RETR_EXTERNAL),
      sortBySize(false) {
  resetMinArea();
  resetMaxArea();
}
...
void ContourFinder::findContours(Mat img) {
  ...
  bool needMaxFilter = maxAreaNorm
                           ? (maxArea < 1)
...
void ContourFinder::setSortBySize(bool sizeSort) { sortBySize = sizeSort; }
...
unsigned int ContourFinder::size() const { return contours.size(); }

vector<cv::Point> &ContourFinder::getContour(unsigned int i) {
  return contours[i];
}

ofPolyline &ContourFinder::getPolyline(unsigned int i) { return polylines[i]; }
...
cv::Point2f ContourFinder::getMinEnclosingCircle(unsigned int i,
                                                 float &radius) const {
  cv::Point2f center;
...
bool ContourFinder::getHole(unsigned int i) const { return holes[i]; }
...
RectTracker &ContourFinder::getTracker() { return tracker; }
...
void ContourFinder::setInvert(bool invert) { this->invert = invert; }
...
void ContourFinder::setSimplify(bool simplify) { this->simplify = simplify; }
...
void ContourFinder::resetMinArea() { setMinArea(0); }
