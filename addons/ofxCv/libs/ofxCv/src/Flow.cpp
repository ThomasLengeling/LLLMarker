...
int Flow::getWidth() { return curr.cols; }
int Flow::getHeight() { return curr.rows; }
void Flow::resetFlow() {
  last = Mat();
  curr = Mat();
  hasFlow = false;
...
FlowPyrLK::FlowPyrLK()
    : windowSize(32), maxLevel(3), maxFeatures(200), qualityLevel(0.01),
      minDistance(4), pyramidLevels(10), calcFeaturesNextFrame(true) {}

FlowPyrLK::~FlowPyrLK() {}

void FlowPyrLK::setWindowSize(int winsize) { this->windowSize = winsize; }
void FlowPyrLK::setMaxLevel(int maxLevel) { this->maxLevel = maxLevel; }
...
void FlowPyrLK::calcFlow(Mat prev, Mat next) {
  if (!nextPts.empty() || calcFeaturesNextFrame) {
    ...
    (CV_MINOR_VERSION > 4 ||                                                   \
     (CV_MINOR_VERSION == 4 && CV_SUBMINOR_VERSION >= 1))
    if (prevPyramid.empty()) {
      buildOpticalFlowPyramid(prev, prevPyramid,
                              cv::Size(windowSize, windowSize), 10);
    }
    buildOpticalFlowPyramid(next, pyramid, cv::Size(windowSize, windowSize),
                            10);
    calcOpticalFlowPyrLK(prevPyramid, pyramid, prevPts, nextPts, status, err,
                         cv::Size(windowSize, windowSize), maxLevel);
    prevPyramid = pyramid;
    pyramid.clear();
#else
    calcOpticalFlowPyrLK(prev, next, prevPts, nextPts, status, err,
                         cv::Size(windowSize, windowSize), maxLevel);
#endif
...
void FlowPyrLK::calcFeaturesToTrack(vector<cv::Point2f> &features, Mat next) {
  goodFeaturesToTrack(next, features, maxFeatures, qualityLevel, minDistance);
}

void FlowPyrLK::resetFeaturesToTrack() { calcFeaturesNextFrame = true; }
...
void FlowPyrLK::drawFlow(ofRectangle rect) {
  ...
  for (int i = 0; i < (int)prevPts.size(); i++) {
    if (status[i]) {
      ofDrawLine(toOf(prevPts[i]) * scale + offset,
                 toOf(nextPts[i]) * scale + offset);
    }
  }
}
...
FlowFarneback::FlowFarneback()
    : pyramidScale(0.5), numLevels(4), windowSize(8), numIterations(2),
      polyN(7), polySigma(1.5), farnebackGaussian(false) {}

FlowFarneback::~FlowFarneback() {}

void FlowFarneback::setPyramidScale(float scale) {
  if (scale < 0.0 || scale >= 1.0) {
    ofLogWarning("FlowFarneback::setPyramidScale")
        << "setting scale to a number outside of 0 - 1";
  }
  this->pyramidScale = scale;
}

void FlowFarneback::setNumLevels(int levels) { this->numLevels = levels; }
void FlowFarneback::setWindowSize(int winsize) { this->windowSize = winsize; }
void FlowFarneback::setNumIterations(int interations) {
  this->numIterations = interations;
}
void FlowFarneback::setPolyN(int polyN) { this->polyN = polyN; }
...
void FlowFarneback::calcFlow(Mat prev, Mat next) {
  ...
  calcOpticalFlowFarneback(prev, next, flow, pyramidScale, numLevels,
                           windowSize, numIterations, polyN, polySigma, flags);
}
